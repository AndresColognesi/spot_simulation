    ...
    def guide_cb(self, req):
        ...
        try:            
            ...
            if (chr(req.dir_command) == 'F'):
                self.SwingPeriod = 0.25 #0.2
                self.StepVelocity = 1.1 #0.001
                self.StepLength = 0.11 #0.045
                self.LateralFraction = 0.0
                self.YawRate = 0.0
                self.ClearanceHeight = 0.06 #0.045
                self.PenetrationDepth = 0.003	  
            elif (chr(req.dir_command) == 'L'):
                self.SwingPeriod = 0.25 #0.2
                self.StepVelocity = 1.1 #0.001
                self.StepLength = 0.055 #0.011
                self.LateralFraction = 0.0
                self.YawRate = 3.5 #2.0
                self.ClearanceHeight = 0.06 #0.045
                self.PenetrationDepth = 0.003	
